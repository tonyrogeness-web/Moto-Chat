...
function getRealPool() {
  ...
  if (process.env.DATABASE_URL) {
    ...
    const mockDbPath = path ? path.join('/tmp', 'mock-db.json') : '';
    if (fs && !fs.existsSync(mockDbPath)) fs.writeFileSync(mockDbPath, '[]');
    ...
    _realPool = {
      ...
      async query(text, params = []) {
        ...
        if (text.includes('DELETE FROM entregas WHERE created_at')) {
          ...
          if (fs) fs.writeFileSync(mockDbPath, JSON.stringify(rem, null, 2));
